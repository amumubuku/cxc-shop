<template>
  <div class="my-coupon">
    <div class="coupon-wrp">
      <ul class="coupon-list" v-if="!couponType">
        <li class="item" v-for="(item, index) in couponList" :key="index">
          ...
          <div class="coupon-label">
            <img class="location-icon" src="../../../static/images/use-coupon.png" alt />
          </div>
        </li>
      </ul>
      ...
      <div class="invalid-coupon" v-if="couponList.length">没有更多无效券了</div>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    getCoupon () {
      ...
      this.$http
        .post('/myCoupons', { is_platform: couponType, status: 'invalid' })
        .then(res => {
