<template>
  <div class="my-popup" :style="{'display': componentDisplay}" :class="className">
    <div class="content-wrp" :class="contentClass" @transitionend="transitionEnd">
      <slot></slot>
      <!-- 由于mpvue的问题，slot的作用域还是在这个组件内。所以暂时不支持动态渲染。 -->
    </div>
    <div class="mask" @click="clickMask()" :class="maskClass" :style="{backgroundColor: maskColor}"></div>
  </div>
</template>
